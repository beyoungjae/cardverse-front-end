...
// 템플릿별 리뷰 조회
export const fetchTemplateReviews = createAsyncThunk('reviews/fetchTemplateReviews', async (templateId, { rejectWithValue }) => {
   try {
...
// 리뷰 수정
export const updateReview = createAsyncThunk('reviews/updateReview', async ({ reviewId, reviewData }, { rejectWithValue }) => {
   try {
...
export const fetchUserReviews = createAsyncThunk('reviews/fetchUserReviews', async (userId, { rejectWithValue }) => {
   try {
      return await reviewApi.getUserReviews(userId)
   } catch (error) {
      return rejectWithValue(error.response?.data || error.message)
   }
})
